...
fdu = conv2(double(dIdu), double(h), 'same'); %add double(), and so were the others conv...
fdv = conv2(double(dIdv), double(h), 'same');
...
% sigma = sqrt(sum(sum(bxfun(@times,double(Isub),r2)))/sum(Isub(:)))/2;  % second moment is 2*Gaussian width, change Isub to double(Isub) 
